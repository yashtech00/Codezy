...
export default function Hero() {
  const installUrl =
    'https://github.com/apps/autoreview-bot/installations/new';
...
Keep DemoPreview.tsx and HowItWorks.tsx as static content — a GIF/screenshot and a 3-step list. No backend calls needed on this page at all.
4. Part B — Dashboard (Week 8+, build only when needed)
...
RepoList and ReviewsTable are plain presentational components — they just map over data and render a table/list. No special logic worth detailing until real data shapes are settled.
5. Backend Change Needed to Support This
...
•	Add two simple REST endpoints: GET /api/reviews (list recent reviews) and GET /api/reviews/:id (single review detail) — reading from the pr_reviews and agent_runs tables already planned in the Week 1–5 schema.
6. Priority Reminder
